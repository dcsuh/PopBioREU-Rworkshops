...
sim2 <- lsoda(y = y0, times = t.vector, func = ExponentialGrowth, parms = parameters2) # complete the arguments 
```
...
#ggplot(data = sim1, aes(x = time, y = 1)) +  geom_point() # complete the code with the appropriate data, and x-y aesthetics 
```
...
ggplot(data = sim1, aes(x = time, y = N0)) + 
 ...
 # we can add points and lines from sim2 by calling the sim2 data, and defining the aesthetic
  geom_point(data = sim2, aes(x=time, y=N0 )) +  
  geom_line(data = sim2, aes(x=time, y=N0 ))
